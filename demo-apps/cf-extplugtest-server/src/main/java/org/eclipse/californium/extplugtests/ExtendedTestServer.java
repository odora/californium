		...
		try {
			...
			if (config.cluster != null) {
				...
				if (config.cluster.clusterType.k8sCluster != null) {
					...
					k8sGroup = new K8sManagementDiscoverClient(config.cluster.clusterType.k8sCluster.externalPort);
			...
			if (k8sGroup != null) {
				...
				for (ClusterNode cluster : config.cluster.clusterType.simpleCluster.dtlsClusterNodes) {
					if (cluster.dtls != null) {
						DtlsClusterConnectorConfig clusterConfig = DtlsClusterConnectorConfig.builder(clusterConfigBuilder.getIncompleteConfig())
								.setAddress(cluster.cluster).build();
						if (config.cluster.clusterType.simpleCluster.dtlsClusterGroup != null) {
