...
public class DtlsClusterConnector extends DTLSConnector {
	...
	private DtlsClusterConnector(DtlsConnectorConfig configuration, DtlsClusterConnectorConfig clusterConfiguration,
		...
		LOGGER.info("cluster-node {}: on {}", getNodeID(), StringUtil.toDisplayString(clusterInternalSocketAddress));
	}
		...
		for (int i = 0; i < receiverThreadCount; i++) {
			Worker receiver = new Worker(
				...
				public void doWork() throws Exception {
					...
					if (!processDatagramFromClusterNetwork(clusterPacket)) {
	...
	protected boolean processDatagramFromClusterNetwork(DatagramPacket clusterPacket) throws IOException {
		...
		if (type != RECORD_TYPE_INCOMING && type != RECORD_TYPE_OUTGOING) {
			return false;
		}
			...
			if (length > Record.RECORD_HEADER_BYTES) {
				...
				if (cid != null) {
					...
					if (getNodeID() != incomingNodeId) {
						...
						if (clusterNode != null) {
							DatagramPacket clusterPacket = encode(packet, RECORD_TYPE_INCOMING);
		...
		if (router != null && backwardMessages) {
			if (nodesProvider.available(router)) {
				...
				LOGGER.trace("cluster-node {}: backwards send message for {} to {}, {} bytes", getNodeID(), destination,
						router, length);
				DatagramPacket clusterPacket = encode(datagram, RECORD_TYPE_OUTGOING);
	...
	private DatagramPacket decode(DatagramPacket packet) {
		...
		if (offset == 0) {
			...
			LOGGER.debug("cluster-node {}: packet {} bytes, misformed!", getNodeID(), length);
		}
		return null;
	}
